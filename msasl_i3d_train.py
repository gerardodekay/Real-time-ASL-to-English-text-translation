...
def run_i3d_pretrained():
    EPOCHS = 50
    with open("/home/anilsrik/anil/cs230/data/MSASL_classes.json") as f:
        classes = json.load(f)
    ...
    classes = ['bath', 'calculus', 'israel', 'tall', 'woman', 'hospital', 'sleep', 'knife', 'hungry', 'snowboard',
               ...
               'girlfriend']
    train = keras_video.VideoFrameGenerator(classes=classes, nb_frames=24, batch_size=5, glob_pattern=train_glob_pattern)
    val = keras_video.VideoFrameGenerator(classes=classes, nb_frames=24, batch_size=5, glob_pattern=val_glob_pattern)
    m_rgb = PreTrainedInception3d(include_top=False, pretrained_weights="rgb_imagenet_and_kinetics", dropout_prob=0.5,
                                  input_shape=(24, 224, 224, 3), classes=100)
